...
| 要求 | 实现 |
|---|---|
| 据点争夺逻辑 | `ABattleSectorAnchor`：Overlap 计数 + 人数差推进（攻多→涨 / 守多→退 / 对峙→冻结 / 空区→回退），占领进度 [-1,1] 连续模型 |
| 攻守双方出生 | 双 `ASpawnAreaHub`（攻 -3000 / 守 +3000），GameMode 按加入顺序自动分队（先攻后守） |
| 多人在线同步 | Character / Anchor / GameState 全量 Replicated：移动、朝向、跳跃、占点变色、比分倒计时双窗口一致 |
| 出生点 / UI | 出生点随机选取 + 区域线框标识；正式 HUD（见下） |
| 蓝图 | `BP_GameMode_Breakthrough` / `BP_GameMode_Conquest` / `BP_BattleSectorAnchor` / `BP_SpawnAreaHub` 等 |
