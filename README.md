...
| W10 | 8/25–8/28 | **联机架构重构**：监听服务器 → 专用服务器（DS 构建目标、复制补全）；网络通信两代架构定稿 |
...
### 网络通信：两代联机架构（监听服务器 → 专用服务器）

多人联机随项目演进走了两代拓扑：

**V1 监听服务器（Listen Server）**：一个客户端以 `?listen` 启动即成为主机——该进程既跑全部服务器判定（GameMode、占点、禁区淘汰），又渲染本地玩家画面；其余玩家 `open <主机IP>` 直连（交付包 `Play_Local_2P.bat` 即此模式：先弹主机窗口，3 秒后客户端连入）。开发期最快路径，但存在三个结构性问题：主机进程零延迟（主机优势）、主机掉线全房间崩（服务器寄生在玩家进程里，无独立生命周期）、与正式多人射击游戏的房间服架构脱节。

**V2 专用服务器（Dedicated Server）**：架构评审后重构——服务器独立为无渲染、无本地玩家的进程（`TargetType.Server` 构建目标），所有客户端平等连接；本地开发测试即 `open 127.0.0.1:7777`，变的不是"连哪"，是"连的对象不再是另一个玩家"。同步手段两代不变：服务器→客户端状态走属性复制（`Replicated` + OnRep 表现），请求/事件类走 RPC（角色移动同步引擎自带）。

| | V1 监听服务器 | V2 专用服务器（DS） |
|---|---|---|
| 服务器宿主 | 某个玩家进程（既算判定又渲染） | 独立无渲染进程 |
| 公平性 | 主机 0 延迟 | 所有客户端等延迟 |
| 生命周期 | 主机掉线 = 全房间崩 | 服务器独立存活 |

**迁移成本几乎为零**：玩法从第一天坚持「判定只在服务器（`HasAuthority()` 保护）、表现全靠 OnRep 回调」——拓扑换了，玩法代码几乎一行不动，换的只是"服务器住在哪个进程里"。迁移同时补齐了区内人数的复制（监听服务器时代主机窗口恰好就是服务器、能看到正确数据，把这个缺口掩盖了；DS 下所有客户端都是纯客户端，必须正式 `Replicated`）。
