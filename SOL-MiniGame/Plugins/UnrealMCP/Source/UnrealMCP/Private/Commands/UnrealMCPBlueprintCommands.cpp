...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCreateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!ParentClass.IsEmpty())
    {
        ...
        else
        {
            ...
            // SOL: this project's gameplay module. Without it a create_blueprint
            // with parent_class="SOLExtractionZone" only resolved through the
            // loaded-class fallback scan; listing the module makes it deterministic.
            CandidateModules.Add(TEXT("SOL"));
